...
if ($action === 'reports.overview') {
    ...
    $returnsDetail = $safeAll("SELECT r.id, r.return_code, r.created_at, r.reason, r.total_amount, s.name store_name, o.order_code, u.full_name staff_name, (SELECT COUNT(*) FROM return_items ri WHERE ri.return_id=r.id) item_count, (SELECT GROUP_CONCAT(CONCAT(p.name, ' (SL: ', ri.quantity, ', Tiền: ', REPLACE(FORMAT(ri.amount, 0), ',', '.'), ' ₫)') SEPARATOR ' | ') FROM return_items ri JOIN products p ON p.id=ri.product_id WHERE ri.return_id=r.id) items_summary FROM returns r JOIN stores s ON s.id=r.store_id JOIN orders o ON o.id=r.order_id JOIN users u ON u.id=r.created_by WHERE r.store_id IN($ph) ORDER BY r.id DESC LIMIT 30", $ids);
